function export4web(root,paths,callback){
	...
	loader.load(currentPathd,function(content){
		...
		if(m){
			...
			//complete
			callback(impls,internalDeps,externalDeps);
...
function buildExportSource(base,impls,ns,publicCount){
	var result = [];
	var source = exportExample.replace('__dirname',base).replace(/\breturn\s+internal_require\s*\(\s*0\s*\)/,function(a){
		var buf = [];
		if(ns){
			if(publicCount == 1){
				return a;
