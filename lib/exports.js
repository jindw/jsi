var compressJS0 = require('./js-token').compressJS
...
function compressJS(s){
	if(exports.format >0){
		return s;
	}else if(exports.format <0){
		return compressJS0(s);
	}else{
		return s;
	}
}

function buildExportSource(paths,impls,exportCount,exportNS,exportRequire){
...
function exportModules(root,modules,callback,exportRequire){
	buildDefines(root,modules,function(defines,exportModules,internalDeps,externalDeps){
		...
		var result = buildExportSource(modules,defines,modules.length,'',true);
...
function doExport(base,paths,onComplete,exportRequire){
	...
	if(isSingleHTMLFile){
		...
		exportSingleFile(loader,paths[0], onComplete)
	}else{
		exportModules(loader,paths,onComplete,exportRequire)
	}
}
/**
 * <ul>
...
function exportSingleFile(loader,file,callback){
	if(/#/.test(file)){//假文件
		var path = '#main'
	...
	function onExport(defines,paths,internalDeps,externalDeps){
		...
		var mainSource = defines.shift();
		var replaceMap = {};
		for(var i=0;i<defines.length;i++){
			replaceMap[defines[i].path]=i;
		}
		mainSource = mainSource.toString(replaceMap).replace(/[^{]+\{([\s\S]*)\}\s*\);?/,'$1');
		
		//console.warn(mainPath,mainSource)
		mainSource =compressJS (mainSource,mainPath);
		if(internalDeps.length + externalDeps.length ==0){
			callback(mainSource);
		}else{
			var result = buildExportSource(paths,defines,0,'',true) + '\n'+mainSource;
			callback(result);
		}
	}
}
						...
						for(var n in depMap){
							...
							if(!(dep in exportsMap)){
								if(newExported.indexOf(dep)==-1){
									var isInternal = prefixed.indexOf(dep.replace(/\/.*/,''));
