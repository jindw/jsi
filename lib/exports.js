...
function buildExportSource(baseDir,paths,impls,exportCount,exportNS,useRequire){
	...
	//console.log(source)
	source = compressJS(source.replace('__dirname','"./"')+'()').replace(/\(\);?$/,'');
...
function exportSingleFile(root,file,type,callback){
	...
	exports.export4web(root,modules,function(impls,paths,internalDeps,externalDeps){
		if(type == 1 || type==3){
			...
			mainSource = compressJS(mainSource);
			if(paths.length==1 && externalDeps.length ==0){
				callback(mainSource);
				return;
			}
			...
			var result = buildExportSource("'./'",paths,impls,1,'',true);
			callback(result,error);
		}
	})
}
