 ...
 */
function IncludeBuilder(text,file,onComplete){
	this.text = text;
	this.file = file;
	this.deps = file?[file]:[];
...
IncludeBuilder.prototype.toString = function(){
	if(this.complete){
		...
		return mergeSync(this,this.file,this.text)
	}
}
function mergeSync(builder,file,text){
	console.error(file)
	return text.replace(/^\/\/\s*#include\b\s*[('"]+([\/\w\.\-]+?)[)"']+/mg,function(a,includeFile,index){
		includeFile = normalizePath(includeFile,file);
		builder.deps.push(includeFile)
		var content = syncLoadFile(includeFile);
		return mergeSync(builder,includeFile,content);
	})
}
function syncLoadFile(file){
	try{
...
function scheduleLoad(builder,onComplete){
	...
	if(i){
		function loadComplete(content,path,file){
			builder.deps.push(file)
			//console.log(path,content)
			new IncludeBuilder(content,file,function(content,file){
				//console.log(path,content.length)
				builder.contents[path] = content;
				builder.deps.push.apply(builder.deps,this.deps)
				if(--len ==0){
...
function loadBuildInSource(path,callback){
	...
	try{
		switch(path){
		case 'jquery.js':
			var file = require.resolve('../assets/resource/jquery_1_9_1.js');
