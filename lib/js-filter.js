...
function setupJSRequire(rbs, scriptBase, htmlPattern){
...
function buildRequire(rbs,resource,text,scriptBase){
	var path = resource.path;
	var sourcePath = path.replace(/__define__(?=\.js$)/,'');
	var text = merge(rbs,resource,sourcePath,text,scriptBase)
	if(sourcePath != path){
		var path = sourcePath.substr(scriptBase.length).replace(/\.js$/,'');
