...
function ENV(root){
	...
	function load(path,prefix,postfix,lastIndex){
		...
		var resource = instance.resourceMap[path] || 
				( instance.resourceMap[path] = new Resource(instance,path,prefix,postfix));
		...
		for(var i=0;i<=lastIndex;i++){
			if(content[i]===undefined){
				...
				if(pf[0].test(path)){
					data = _require(env,resource,data,i,prefix,postfix);
	...
	function _require(env,resource,data,i,prefix,postfix){
		if(data === undefined){
			data = _getDataAsBinary(resource.sourcePath || resource.path,prefix,postfix);
		}
	...
	function  _defaultSerializeFilter(resource,data){
		if(data == null){
			return _getDataAsBinary(resource.sourcePath || resource.path,resource.prefix,resource.postfix);
		}
	...
	function _translate(env,res,data,translator,cache){
		...
		return cache[i] = translator.apply(env,res,data);
	}
	/**
	 * @public
	 * @param path 文件路径
	...
	function getContent(path,prefix,postfix,translator){
		...
		if(translator){
			return _translate(this,resource,data,translator,resource.cache)
		}
		return  data;
		
	}
	function getContentAsBinary(path,prefix,postfix){
		return getContentAs(path,prefix,postfix,_serialize);
	}
	
	function getContentHash(path,prefix,postfix){
		return getContentAs(path,prefix,postfix,_hash);
	}
	
	
	
	function _getDataAsBinary(path,prefix,postfix){
		var key = [prefix,postfix].join('::');
		var filepath = vfs.getFile(Path.resolve(root,path),key);
		var data = binaryMap[filepath];
		fileMap[key+path] = filepath;
		if(!data){
			data = binaryMap[filepath] = FS.readFileSync(filepath)
		}
		return data
	}
	function _onChange(key,file){
		console.info("clean:",file);
		var path = Path.relative(root,file).replace(/^\w/,'/$&')
		var instance = instanceMap[key];
		delete binaryMap[fileMap[key+path]];
		if(instance){
