...
function _addFilter(pattern,impl,type){
	...
	switch(type){
		case BINARY_BUILDER:
			item = globalsFilters.splice(textBuilderStart++,0,item) , null;
		case TEXT_BUILDER:
			item = item && globalsFilters.splice(textBuilderStart,0,item) , null;
			textFilterStart++;
		case TEXT_FILTER:
			item = item && globalsFilters.splice(domBuilderStart,0,item) , null;
			domBuilderStart++;
		case DOM_BUILDER:
			item = item && globalsFilters.splice(domFilterStart,0,item) , null;
			domFilterStart++;
		case DOM_FILTER:
			break;
		case SERIALIZER:
	...
	try{
		...
		for(var i=0;i<lastIndex;i++){
			if(cache[i]!==undefined){
				...
				if(pf[0].test(path)){
					if(data === undefined){
						data = VFS.getDataAsBinary(path,prefix,postfix);
					}
