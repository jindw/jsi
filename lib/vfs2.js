...
function _on_dir_changed(dir,newFileMap){
	...
	for(var tryPath in failedMap){//add
		if((tryPath in newFileMap) && !(tryPath in oldFileMap)){//new File
			var tryPathMap = failedMap[tryPath];//tryPath=>{id=>path}
			var pathMap = {};
			for(var path in tryPathMap){
				pathMap[path]=1;
			}
			for(var truePath in hitMap){
				var hitPathMap = hitMap(truePath)
				for(var hitId in hitPathMap){
					if(hitPathMap[hitId] in paths){
						//listener(hitId,hitPathMap[hitId]);
						_clean_cache(hitMap,truePath);
						break;
					}
				}
			}
...
function _dir_removed(dir){//dir removed
	...
	for(var truePath in dir.hitMap){
		_clean_cache(dir.hitMap,truePath);
	}
}
function _on_file_change(dir){//file modified,delete
	...
	for(var truePath in hitMap){
		try{
			var absPath = Path.resolve(dir.path,truePath);
			var oldStat = fileMap[truePath];
			...
			console.log(dir.path,truePath,absPath,oldStat==null,newStat==null)
			console.warn('file statSync failed',e);
		}
		_clean_cache(hitMap,absPath);
...
function _clean_cache(hitMap,truePath){
	var pathMap = hitMap[truePath]
	for(var id in pathMap){
		listener(id,pathMap[id]);
	}
	delete hitMap[truePath];
}
}