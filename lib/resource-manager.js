...
var DOM_FILTER = 4;
var SERIALIZER = 5;
function Resource(resourceManager,instance,path,prefix,postfix){
...
Resource.prototype.load = function(path){
	...
	var res = this.resourceManager.load(path,this.prefix,this.postfix,index);
...
Resource.prototype.getExternalAsBinary = function(file,rootPath){
	var key = this.instance.key;
	//console.log(file,rootPath,Path.resolve(rootPath,'.'+file))
	if(rootPath){
		file = this.resourceManager.vfs.getFile(Path.resolve(rootPath,'.'+file),key);
	}
	//console.log(file)
	return this.resourceManager._getDataAsBinary(this.path,key,file)
...
function ResourceManager(rootPath){
	...
	function load(path,prefix,postfix,lastIndex){
		...
		var resource = instance.resourceMap[path] || 
				( instance.resourceMap[path] = new Resource(resourceManager,instance,path,prefix,postfix));
	...
	resourceManager._getDataAsBinary = function(path,key,file){
