...
if(UglifyJS){
	...
	compressJS0 = function(source){
		var code = UglifyJS.minify(String(source), {fromString: true}).code;
		if(!code){
...
function compressJS(s){
	var sample = s.slice(s.length/10,s.length/1.1);
	...
	if(sample.match(/\s{3}/)){
		...
		try{
			...
			s= compressJS0('alert('+s+'\n)').replace(/\balert\(([\s\S]*)\);?\s*$/,'$1') ;
		}
		//console.log('\&&&&\n',s,"====");
	}
	return s;
...
function partitionJavaScript(source){
	...
	/* */
	var regexp = /'(?:\\.|[^'])*'|"(?:\\.|[^"])*"|\/\/.*|\/\*([^*]+|\*[^\/])*\*\/|\/(?=.+\/(?:[img]*\b|[^\w]))|<(?=(?:[A-Za-z_][\w_\-\.]*(?:\:[\w\-\.]*)?)(?:\s*\/?>|\s+\w))/i;
