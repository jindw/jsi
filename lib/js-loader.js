	...
	loader(sourcePath,function(file,realPath,text){
		...
		var text = new IncludeBuilder(realPath,text,loader,function(){
			if(isDefine){
				...
				var realId = realPath.replace(/(.\/.+)\.js$/,'$1');
				if(rawId!=realId){
...
function restoreFromErr(loader,file,path,callback,idIndex){
	...
	//if((/\.css(?:[#?].*)?$/.test(id) )
	var subFileMatchs = path.match(/^([\w\.\-]+)(\/.*)?$/);//		module, module/files
	if(subFileMatchs){
		...
		if(module.substr(0,root.length) == root){
			...
			if(m && m[2] && m[1] != 'node_modules'){
				//inline module export!
				var exports = require('./exports');
				exports.export4web(loader.root,[module],function(impls,internalDeps,externalDeps){
					console.log.apply(console,arguments);
					var source = exports.buildExportSource('__dirname',impls,'',1)
