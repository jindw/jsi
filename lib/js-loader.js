...
ScriptLoader.prototype.load = function(path,callback,idIndex){
	...
	loader(sourcePath,function(file,realPath,text){
		...
		if(text instanceof Array){
			...
			text = text.replace(/^\ufeff/,'')
			if(/\.xml$/.test(file) && /^</.test(text)){//xml
				text = 'module.exports = exports.xml='+JSON.stringify(text);
			}
	...
	if(subFileMatchs){
		try{
			...
			if(/^jquery$/i.test(subFileMatchs[1])){
				...
				callback(file,path,'console.log('+JSON.stringify("module not found!!!"+path)+')');
			}
			return;
		}
		if(module){
			...
			if(module.substr(0,root.length) == root){
				var module2 = module.substr(root.length)
				var m = module2.match(/^([\w\.\-]+)(\/.*)$/);
				//console.log(module,path)
				
				
				module2 = module2.replace(/\.js$/,'');
				if( module2 != path.replace(/\.js$/,'') && m && m[2] && m[1] != 'node_modules'){
					if(idIndex){//merge outer
						//continue!!
					}else if(subFileMatchs[2] == '.js' || !subFileMatchs[2]  ){//index module
						//inline module export!
						var exports = require('./exports');
						exports.export4web(loader.root,[module2],function(impls,idIndex,internalDeps,externalDeps){
							var source = impls;//.join(',')
							//console.log('export:',module2,impls.length)
							callback(file,module2,source)
						})
						
						return;
					}else if(subFileMatchs[2] == m[2]){//sub module
						//return;
					}
				}
				var dir = root + module2.replace(/^((?:node_modules\/)?[\w\.\-]+)\/?.*$/,'$1');
			}else{
			...
			var file = dir + (modulePath ||  module.substr(dir.length)).replace(/^\/?(?=.)/,'/')
