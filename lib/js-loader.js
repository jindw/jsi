...
var formatJS = require('./js-token').formatJS
...
function transformCode(source,path,format){
	...
	default:
		if(format){
			console.warn('unsupport format type:'+format+'; only support:format,compressed,raw')
		}
		return source;
	}
	}catch(e){
...
ScriptLoader.prototype.findModuleAndPath = function(id,baseFile){
	...
	var fileMatchs = id.match(/^(\w[\w\.\-]*?)(\/.*|\.js)?$/);
	...
	do{
		for(var i=0;i<bases.length;i++){
			...
			if(dir.indexOf(config.base)==0){
				...
				if(i ==0 && main == null){
					main = config.moduleMap[moduleName.replace(/\.js$/i,'')];
				}
...
ScriptLoader.prototype.loadFile = function(path,callback){
	this.loader(path,function(text,path,file){
		...
		new IncludeBuilder(text,function(text){
			callback(text,path,file);
		},file);
	})	
}
