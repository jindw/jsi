...
function ScriptLoader(root,loader){
	this.loader = loader || buildLoader(root||'./');
}
ScriptLoader.prototype.load = function(path,callback,pathIndex){
	//console.log(path)
	var sourcePath = path.replace(/__define__(?=\.js$)/,'');
	var isDefine = sourcePath != path;
	var loader = this.loader;
	loader(sourcePath,function(path,text){
		var text = new IncludeBuilder(path,text,loader,function(){
			if(isDefine){
				var id = path.replace(/\.js$/,'');
				callback( buildDefine(id,text,pathIndex));
			}else{
				callback( text);
			}
		});
