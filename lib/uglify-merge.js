...
exports.runMerge = function(paths,opt){
	...
	compressed = compressed.print_to_string({beautify:true});
	return (compressed)
}
	...
	if(invalidRequires.length ==0 && invalidExports.length ==0){//do replace!!
		...
		function afterReplacer(node){
			...
			if(isExportProperty(node)){
				...
				return symbolRef;
			}else if(node instanceof UglifyJS.AST_Assign && node.operator == '='){
