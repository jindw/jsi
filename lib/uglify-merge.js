...
exports.runMerge = function(paths,opt){
	var globals = {};
	var varIndex = 0;
	var numberRadix = numbers.length;
	function nextGlobals(module,rawId){
		if(DEBUG){
			return module.replace(/[^\w]/g,'_')+'$'+rawId;
		}
		while(++varIndex%numberRadix>=numberRadix-10);
		var result = [];
		var left = varIndex;
		do{
			var c = left%numberRadix;
		...
		if(RESERVED_WORDS.indexOf(result)>=0){
			return nextGlobals(numberRadix);
		}else{
			return result;
		}
	}
	...
	while(i--){
		...
		for(var n in info.exportMap){
			...
			mangledVars[realName] = v.mangled_name = nextGlobals(id,realName);
		...
		info.exportAst.variables.each(function(v,n){
			if(!v.mangled_name){
				...
				if(requireVarInfo){
					...
					v.mangled_name = nextGlobals(id,v.name);
				}
			}
		})
