...
exports.cmd= function cmd(args){
	...
	if(cmd == 'install'){
		...
		var isSingleFile = args.length == 1 && /\.js$/.test(args[0]);
		if(isHelp){
			...
			if(isSingleFile){
				exports.exportSingleFile('./',args[0],type || 1, onComplete)
			}else{
				exports.exportModules('./',args,type,onComplete)
			}
		}
