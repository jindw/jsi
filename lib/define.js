...
function buildDefine(root,file,id,text,idIndex,mainMap){
	text = text.replace(/\r\n?/g,'\n');
	var deps = [];
	text  = buildDependence(root,file,id,text,deps,idIndex,mainMap)
	...
	result.push('){');
	if(hitfilename && idIndex){
		var index = idIndex.indexOf(id);
		var refId= mainMap[id] || id;
		var p = refId.indexOf('/');
		if(p >0){
			var pathname = refId.substring(p)+'.js';
		}else{
			var pathname = '/';
		}
		result.push('__filename = __filename.replace(/\\/\d+$/,"'+pathname+'");')
	}
...
function buildDependenceAndXML(root,file,id,text,deps,idIndex,mainMap){
				...
				if(match){
					if(match[1] == 'require'){
						if(item!= (item=item.replace(/^(['"])([\w\.\/-]+)\1/,'$2'))){
							if(idIndex && item.charAt(0) == '.' //相对路径替换成数字
								||  mainMap[item.replace(/\/.*$/,'')]//?
								){
								//替换成数字
								item = absoluteModule(item,id)
								var index = idIndex.indexOf(item);
								if(index>=0){
									tokens[i] = index;
									deps.push(index);
								}else{
									index = idIndex.push(item)-1;
									tokens[i] = index;
									deps.push(index);
								}
...
function buildDependence(root,file,id,text,deps,idIndex,mainMap){
	if(lite || idIndex){
		return buildDependenceAndXML(root,file,id,text,deps,idIndex,mainMap);
	}
	var sourceToken = [];
//	var targetTokens = [];
	var end=0;
