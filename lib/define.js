...
function parseDependence(content,root,file){
	...
	function parseLite(item,prev){
		...
		if(functionQute){
			tokens[i-1] = prev+params+'{'+fnCode.replace(/^[^{]+.|\}[^}]*$/g,'')
		}else{
			var m = fnCode.match(/function\(\)\{\s*return/);
			if(m){
				tokens[i-1] = prev+fnCode.substring(m[0].length).replace(/;?\s*\}[^}]*$/,'')
			}else{
				tokens[i-1] = prev+'('+fnCode+')()';
			}
			
		}
		tokens[i] = '';
		tokens[i+1] = tokens[i+1].replace(/^\s*\)/,'')
	}
