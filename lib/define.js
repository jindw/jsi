...
function buildDefine(id,text,idIndex,mainMap){
	text = text.replace(/\r\n?/g,'\n');
	var deps = [];
	text  = buildDependence(id,text,deps,idIndex,mainMap)
...
function buildDependenceAndXML(id,text,deps,idIndex,mainMap){
				...
				if(match){
					if(match[1] == 'require'){
						if(item!= (item=item.replace(/^(['"])([\w\.\/-]+)\1/,'$2'))){
							if(idIndex && item.charAt(0) == '.' ||  mainMap[item.replace(/\/.*$/,'')]){
								//替换成数字
...
function buildDependence(id,text,deps,idIndex,mainMap){
	if(lite || idIndex){
		return buildDependenceAndXML(id,text,deps,idIndex,mainMap);
	}
	var sourceToken = [];
//	var targetTokens = [];
	var end=0;
