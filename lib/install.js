...
var export4web = require('./build').export4web
var execPath = process.execPath; //usr/local/bin/node
try{
	var UglifyJS = require("uglify-js");
	var compressJS = function(source){
		...
		if(!exists || force){
			fs.unlink(boot,function (err) {
				...
				loader.load('boot.js',function(content){
					fs.writeFile(boot,compressJS(String(content)),function(err){
						if(err){
...
function updateConfig(root,list,callback){
	...
	fileutil.mkdirs(path.dirname(mainConfig),function(){
		fs.readFile(mainConfig,function(err,data){
			var json = JSON.parse(data?data.toString().replace(/^[^{]+(\{[\s\S]+\})[^}]+$/,'$1'):'{}');
			var i = list.length;
			while(i--){
...
function compileModule(root,nameVersions,sourcePath,callback){
	...
	export4web(path.dirname(sourcePath),[name_],function(impls,internalDeps,externalDeps){
		//console.log(impls.length,internalDeps,externalDeps)
		...
		fileutil.copy(sourcePath,dest,function(err){
			...
			fs.writeFile( path.join(dest,'0.js'),compressJS(source),function(err){
