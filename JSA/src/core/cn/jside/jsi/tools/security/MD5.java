package cn.jside.jsi.tools.security;
...
public class MD5 {
	...
	public static void main (String[] args){
		if (args.length == 0){
			...
			for (int i=0; i<args.length; i++){
				try {
					DebugTool.info(MD5.getHashString(new File(args[i])) + " " + args[i]);
				} catch (IOException x){
	...
	public static boolean checkSign(final String data,String key){
		...
		for (int i = 0; i < keys.length; i++) {
			...
			char alpha = key.charAt(key.length()-1);
			value = value.replaceAll("[\\s/\\*]", "").toUpperCase()+ alpha;
			key = key.substring(0,key.length()-1);
			String back = value = getHashString(value);
			while(true){
	...
	private static String toString(byte hash[]){
		...
		for (int i=0; i<hash.length; i++){
			...
			buf.append(Integer.toHexString(((intVal<<2) ^ intVal) & 0xFF ));
