...
function buildRequire(findPackageByPath,realPackage){
...
function writeRequire(packageObject,fileName,object){
	...
	for(var i = 0,l = result.length;i<l;i++){
		document.write("<script src='"+result[i]+"'></script>");
	}
}
...
function requireScript(packageObject,fileName,object){
	if(!packageObject.requireMap){
		packageObject.requireMap = {};
	}
    var loader = packageObject.requireMap[fileName];
    if(!loader){
...
function prepareScriptLoad(packageObject,loader){
    ...
    while(i--){
        ...
        if(dep[4]){//装在后依赖，记录依赖，以待装载
            ...
            if(dep = packageObject.requireMap[name]){
                return dep;
            }
        }
    }
...
function doScriptLoad(packageObject,loader){
    ...
    packageObject.requireMap[loaderName] = loader;
